...
export default function (pi: ExtensionAPI) {
  const SKILLS_DIR = process.env.PI_SKILL_EVOLUTION_DIR ?? join(
    ...
    async execute(_toolCallId, params) {
      ...
      if (operation === "list") {
        if (!existsSync(SKILLS_DIR)) {
          return {
            content: [{ type: "text", text: "No skills directory found. Skills will be created in: " + SKILLS_DIR }],
            details: {},
          };
        }
      ...
      if (err) return { content: [{ type: "text", text: `Invalid skill name "${skillName}": ${err}` }], isError: true, details: {} };
      ...
      if (operation === "inspect") {
        if (!existsSync(skillPath)) {
          return { content: [{ type: "text", text: `Skill "${skillName}" not found at ${skillPath}` }], isError: true, details: {} };
        }
      ...
      if (operation === "create") {
        ...
        if (!desc) return { content: [{ type: "text", text: 'Missing "description" for create' }], isError: true, details: {} };
        if (existsSync(skillPath)) return { content: [{ type: "text", text: `Skill "${skillName}" already exists at ${skillPath}. Use "edit" or "patch" to update.` }], isError: true, details: {} };

        mkdirSync(skillDir, { recursive: true });
        writeFileSync(skillPath, buildSkillMd(skillName, desc, params.content ?? "# " + skillName), "utf-8");
        return {
          content: [{ type: "text", text: `Created skill "${skillName}" at ${skillPath}` }],
          details: { path: skillPath },
        };
      }

      if (operation === "edit") {
        if (!existsSync(skillPath)) return { content: [{ type: "text", text: `Skill "${skillName}" not found. Use "create" first.` }], isError: true, details: {} };
        const desc = params.description;
        if (!desc) return { content: [{ type: "text", text: 'Missing "description" for edit' }], isError: true, details: {} };
        writeFileSync(skillPath, buildSkillMd(skillName, desc, params.content ?? ""), "utf-8");
        return { content: [{ type: "text", text: `Updated skill "${skillName}" at ${skillPath}` }], details: { path: skillPath } };
      }

      if (operation === "patch") {
        if (!existsSync(skillPath)) return { content: [{ type: "text", text: `Skill "${skillName}" not found. Use "create" first.` }], isError: true, details: {} };
        if (!params.find) return { content: [{ type: "text", text: 'Missing "find" for patch' }], isError: true, details: {} };
        const text = readFileSync(skillPath, "utf-8");
        if (!text.includes(params.find)) return { content: [{ type: "text", text: `Patch failed: "${params.find.slice(0, 60)}..." not found in ${skillPath}` }], isError: true, details: {} };
        const count = (text.match(new RegExp(
          params.find
            .replace(/[.*+?^${}()|[\]\\]/g, "\\$&"),
          "g",
        )) ?? []).length;
        if (count > 1) return { content: [{ type: "text", text: `Patch ambiguous: "${params.find.slice(0, 60)}..." found ${count} times. Use a longer, unique snippet.` }], isError: true, details: {} };
        const newText = text.replace(params.find, params.replace ?? "");
        writeFileSync(skillPath, newText, "utf-8");
        return {
          content: [{ type: "text", text: `Patched skill "${skillName}" at ${skillPath}` }],
          details: { path: skillPath },
        };
      }

      if (operation === "delete") {
        if (!existsSync(skillPath)) return { content: [{ type: "text", text: `Skill "${skillName}" not found.` }], isError: true, details: {} };
        ...
        return { content: [{ type: "text", text: `Deleted skill "${skillName}"` }], details: {} };
      }

      return { content: [{ type: "text", text: `Unknown operation: ${operation}` }], isError: true, details: {} };
    },
  });
